...
punkte = np.array([
    ...
    [3035, 188],    # oberer rechte Eckpunkt
    [4666, 2228],   # unterer rechte Eckpunkt
    [2379, 3989]    # unterer linke Eckpunkt
], dtype=np.int32)
...
# eigentlich cotan, aber so ausgedrückt um 1/0 zu vermeiden
for _, angle, dist in zip(*hough_line_peaks(h, theta, d)):
    ...
    q = y0_int - (np.tan(angle + np.pi / 2) * x0_int)
    q_int = int(q)

    # der Farbwert des Fusspunktes
